...
from .config import ctrl_down, gc
...
if anki_point_version >= 50:
    ...
    class EditCurrentModFor50_to_66(EditCurrent):
...
if anki_point_version >= 67:
    from aqt.editcurrent import EditCurrent
    from aqt.utils import disable_help_button, tr

    class EditCurrentModFor2310_or_later(EditCurrent):
        def __init__(self, mw: aqt.AnkiQt) -> None:
            ...
            # self.editor.set_note(self.mw.reviewer.card.note(), focusTo=0)
            self.editor.card = None
            self.editor.set_note(note_to_use, focusTo=0)
            ### mod end
            restoreGeom(self, "editcurrent")
...
def external_note_dialog(note):
    global note_to_use
    if gc("nid link open in browser") or ctrl_down():
        ...
        if anki_point_version <= 49:
            ...
            d = EditCurrentModFor50_to_66(mw, note)
            d.show()
        else:
            note_to_use = note
            d = EditCurrentModFor2310_or_later(mw)
            d.show()
