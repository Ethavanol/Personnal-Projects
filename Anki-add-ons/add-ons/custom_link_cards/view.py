...
def get_repl(cmd, quoted=False):
    shown = gc("Default link text")
    if theme.theme_manager.get_night_mode():
        style = gc("Link text style in reviewer-previewer - night-mode", "")
    else:
        style = gc("Link text style in reviewer-previewer - day", "")   
    if quoted:
        if gc("Hide nidd-cidd string in reviewer-previewer, only show short link"):
            return """<a href='javascript:pycmd(\`%s\\2\`);' style='%s'>%s</a>""" % (cmd, style, shown)
        else:
